...
# ================== WS Candles Aggregator (persistent DB cache) ==================
# The goal: keep candles in Postgres (candles_cache) up-to-date via WebSockets so MID can read fast from DB,
# and REST is only used as fallback (refill) when cache is missing/stale/has gaps.

def _ws_tf_to_minutes(tf: str) -> int:
    t = (tf or "").lower().strip()
    if t in ("1m","1min","1"):
        return 1
    if t in ("3m","3min","3"):
...
def _ws_interval_ms(tf: str) -> int:
    return int(_ws_tf_to_minutes(tf) * 60_000)

def _ws_okx_inst(symbol: str) -> str:
    s = (symbol or "").upper().strip()
    if s.endswith("USDT"):
        return f"{s[:-4]}-USDT"
    return s.replace("/", "-")

def _ws_binance_stream(sym: str, tf: str) -> str:
    return f"{sym.lower()}@kline_{(tf or '').lower()}"

def _ws_okx_channel(tf: str) -> str:
    # OKX uses candle1H not candle1h
    t = (tf or "").strip()
    if t.lower() == "1h":
        t = "1H"
    return f"candle{t}"

def _ws_candles_key(ex: str, market: str, symbol: str, tf: str, limit: int) -> str:
    return f"candles:{ex}:{market}:{symbol}:{tf}:{int(limit)}"

async def _ws_write_df(ex: str, market: str, symbol: str, tf: str, *, limit: int, df: "pd.DataFrame") -> None:
    try:
        key = _ws_candles_key(ex, market, symbol, tf, limit)
        payload = db_store._cc_pack_df(df)
        await db_store.candles_cache_set(key, payload)
    except Exception:
        # best effort; never crash WS loop
        pass

def _ws_df_from_rows(rows: list[list[object]]) -> "pd.DataFrame":
...
class _WSCandleBuffer:
    __slots__ = ("rows", "last_write", "pending")
    def __init__(self):
...
async def ws_candles_service_loop(backend: "Backend") -> None:
    """
    Background task: keep candles_cache warm via WS.

    Reads symbols from backend symbol caches:
      - prefers backend._mid_symbols_cache (if exists and recent)
      - otherwise uses backend._symbols_cache
    Subscribes to Binance/Bybit/OKX public candlestick WS and writes packed DataFrames into db_store.candles_cache_*.

    Safety:
      - Never raises; reconnects on errors.
      - Throttles DB writes (writes on candle close, and at most once per CANDLES_WS_WRITE_EVERY_SEC per key).
    """
    logger = logging.getLogger("crypto-signal")
    enabled = os.getenv("CANDLES_WS_ENABLED", "0").strip().lower() not in ("0","false","no","off")
    if not enabled:
        logger.info("[ws-candles] disabled (CANDLES_WS_ENABLED=0)")
        return

    # config
    ex_list_env = (os.getenv("CANDLES_WS_EXCHANGES", "BINANCE,BYBIT,OKX") or "").strip()
    exchanges = [x.strip().upper() for x in ex_list_env.split(",") if x.strip()]
    if not exchanges:
        exchanges = ["BINANCE","BYBIT","OKX"]

    market = (os.getenv("CANDLES_WS_MARKET", "SPOT") or "SPOT").upper().strip()
    if market not in ("SPOT","FUTURES"):
        market = "SPOT"

    tfs_env = (os.getenv("CANDLES_WS_TFS", "5m,30m,1h") or "").strip()
    tfs = [x.strip() for x in tfs_env.split(",") if x.strip()]
    if not tfs:
        tfs = ["5m","30m","1h"]

    limit = int(float(os.getenv("CANDLES_WS_LIMIT", "250") or 250))
    top_n = int(float(os.getenv("CANDLES_WS_TOP_N", os.getenv("MID_TOP_N", "70") or 70) or 70))
    write_every = float(os.getenv("CANDLES_WS_WRITE_EVERY_SEC", "3") or 3.0)
    resub_check = float(os.getenv("CANDLES_WS_RESUB_CHECK_SEC", "30") or 30.0)

    # internal buffers (key -> buffer)
    buf: dict[tuple[str,str,str,str], _WSCandleBuffer] = {}

    def _get_symbols() -> list[str]:
        now = time.time()
        # prefer MID symbols cache if fresh
        try:
            syms = list(getattr(backend, "_mid_symbols_cache", []) or [])
            ts = float(getattr(backend, "_mid_symbols_cache_ts", 0.0) or 0.0)
            if syms and (now - ts) <= 600:
                return syms[:max(1, top_n)]
        except Exception:
            pass
        try:
            syms = list(getattr(backend, "_symbols_cache", []) or [])
            return syms[:max(1, top_n)]
        except Exception:
            return []

    async def _flush_if_needed(ex: str, sym: str, tf: str) -> None:
        key = (ex, market, sym, tf)
        b = buf.get(key)
        if not b or not b.pending:
            return
        now = time.time()
        if (now - b.last_write) < write_every:
            return
        b.pending = False
        b.last_write = now
        df = _ws_df_from_rows(b.rows[-limit:])
        if not df.empty:
            await _ws_write_df(ex, market, sym, tf, limit=limit, df=df)

    def _upsert_row(ex: str, sym: str, tf: str, *, open_ms: int, o: float, h: float, l: float, c: float, v: float, is_final: bool) -> None:
        key = (ex, market, sym, tf)
        b = buf.get(key)
        if b is None:
            b = _WSCandleBuffer()
            buf[key] = b
        rows = b.rows
        # replace last if same open_time, else append
        if rows and int(rows[-1][0]) == int(open_ms):
            rows[-1] = [int(open_ms), o, h, l, c, v]
        else:
            rows.append([int(open_ms), o, h, l, c, v])
            # keep a bit more than limit to handle reorgs
            if len(rows) > (limit + 50):
                b.rows = rows[-(limit+50):]
        # mark pending on final candle, otherwise only if we have never written
        if is_final or b.last_write <= 0:
            b.pending = True
    ...
    async def _binance_loop():
        ...
        while True:
            try:
                ...
                for s in syms:
                    for tf in tfs:
                        streams.append(_ws_binance_stream(s, tf))
                sig = ",".join(streams)
                async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=10, max_queue=2000) as ws:
                    ...
                    if streams:
                        ...
                        last_sub_sig = sig
                        logger.info("[ws-candles][binance] subscribed streams=%s symbols=%s tfs=%s", len(streams), len(syms), tfs)
                    ...
                    while True:
                        ...
                        if isinstance(data, dict) and data.get("e") == "kline":
                            ...
                            try:
                                ...
                                continue
                            _upsert_row("BINANCE", sym, tf, open_ms=open_ms, o=o, h=h, l=l, c=c, v=v, is_final=is_final)
                            await _flush_if_needed("BINANCE", sym, tf)
                        # periodic resubscribe if symbol set changed
                        now = time.time()
                        if (now - last_check) >= resub_check:
                            last_check = now
                            syms2 = _get_symbols()
                            streams2 = []
                            for s in syms2:
                                for tf in tfs:
                                    streams2.append(_ws_binance_stream(s, tf))
                            sig2 = ",".join(streams2)
                            if sig2 != last_sub_sig and streams2:
                                sub_id += 1
                                try:
                                    await ws.send(json.dumps({"method":"UNSUBSCRIBE","params":streams,"id":sub_id}))
                                except Exception:
                                    pass
                                streams = streams2
                                last_sub_sig = sig2
                                sub_id += 1
                                await ws.send(json.dumps({"method":"SUBSCRIBE","params":streams,"id":sub_id}))
                                logger.info("[ws-candles][binance] resubscribed streams=%s symbols=%s", len(streams), len(syms2))
            except Exception as e:
                logger.warning("[ws-candles][binance] reconnect: %s", str(e)[:200])
                await asyncio.sleep(2.0)

    async def _bybit_loop():
        if "BYBIT" not in exchanges:
            return
        # Bybit V5 public WS: spot uses /v5/public/spot
        url = "wss://stream.bybit.com/v5/public/spot" if market == "SPOT" else "wss://stream.bybit.com/v5/public/linear"
        last_sub_sig = ""
        while True:
            try:
                syms = _get_symbols()
                ...
                for s in syms:
                    for tf in tfs:
                        mins = _ws_tf_to_minutes(tf)
                        args.append(f"kline.{mins}.{s.upper()}")
                sig = ",".join(args)
                async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=10, max_queue=2000) as ws:
                    if args:
                        await ws.send(json.dumps({"op":"subscribe","args":args}))
                        last_sub_sig = sig
                        logger.info("[ws-candles][bybit] subscribed topics=%s symbols=%s tfs=%s", len(args), len(syms), tfs)
                    last_check = time.time()
                    while True:
                        msg = await ws.recv()
                        if not msg:
                        ...
                        if isinstance(data, dict) and ("topic" in data) and ("data" in data):
                            ...
                            if len(parts) >= 3:
                                ...
                                continue
                            # Bybit sends data as dict or list of dicts
                            ...
                            for it in items:
                                if not isinstance(it, dict):
                                    continue
                                try:
                                    open_ms = int(it.get("start") or it.get("startTime") or it.get("t"))
                                    ...
                                    v = float(it.get("volume") or it.get("v") or 0.0)
                                    is_final = bool(it.get("confirm") or it.get("x") or False)
                                except Exception:
                                    continue
                                tf = f"{tf_min}m" if str(tf_min).isdigit() else str(tf_min)
                                if tf == "60m":
                                    tf = "1h"
                                _upsert_row("BYBIT", sym, tf, open_ms=open_ms, o=o, h=h, l=l, c=c, v=v, is_final=is_final)
                                await _flush_if_needed("BYBIT", sym, tf)
                        now = time.time()
                        if (now - last_check) >= resub_check:
                            last_check = now
                            syms2 = _get_symbols()
                            ...
                            for s in syms2:
                                for tf in tfs:
                                    mins = _ws_tf_to_minutes(tf)
                                    args2.append(f"kline.{mins}.{s.upper()}")
                            sig2 = ",".join(args2)
                            if sig2 != last_sub_sig and args2:
                                try:
                                ...
                                last_sub_sig = sig2
                                await ws.send(json.dumps({"op":"subscribe","args":args}))
                                logger.info("[ws-candles][bybit] resubscribed topics=%s symbols=%s", len(args), len(syms2))
            except Exception as e:
                logger.warning("[ws-candles][bybit] reconnect: %s", str(e)[:200])
                await asyncio.sleep(2.0)
        ...
        while True:
            try:
                ...
                for s in syms:
                    inst = _ws_okx_inst(s)
                    for tf in tfs:
                        args.append({"channel": _ws_okx_channel(tf), "instId": inst})
                sig = json.dumps(args, sort_keys=True)
                async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=10, max_queue=2000) as ws:
                    if args:
                        await ws.send(json.dumps({"op":"subscribe","args":args}))
                        last_sub_sig = sig
                        logger.info("[ws-candles][okx] subscribed topics=%s symbols=%s tfs=%s", len(args), len(syms), tfs)
                    last_check = time.time()
                    while True:
                        msg = await ws.recv()
                        if not msg:
                        ...
                        try:
                            ...
                            continue
                        if isinstance(data, dict) and ("arg" in data) and ("data" in data):
                            arg = data.get("arg") or {}
                            ch = str(arg.get("channel") or "")
                            inst = str(arg.get("instId") or "")
                            if not ch.startswith("candle"):
                                continue
                            tf = ch.replace("candle", "")
                            if tf == "1H":
                                tf = "1h"
                            # inst to symbol
                            ...
                            for it in items:
                                # it could be list: [ts, o, h, l, c, vol, ...]
                                if isinstance(it, (list, tuple)) and len(it) >= 6:
                                    try:
                                        open_ms = int(float(it[0]))
                                        o = float(it[1]); h = float(it[2]); l = float(it[3]); c = float(it[4]); v = float(it[5] or 0.0)
                                    except Exception:
                                        continue
                                    # OKX does not always include confirm; treat as pending but still write periodically.
                                    _upsert_row("OKX", sym.upper(), tf, open_ms=open_ms, o=o, h=h, l=l, c=c, v=v, is_final=True)
                                    await _flush_if_needed("OKX", sym.upper(), tf)
                        now = time.time()
                        if (now - last_check) >= resub_check:
                            last_check = now
                            syms2 = _get_symbols()
                            ...
                            for s in syms2:
                                inst = _ws_okx_inst(s)
                                for tf in tfs:
                                    args2.append({"channel": _ws_okx_channel(tf), "instId": inst})
                            sig2 = json.dumps(args2, sort_keys=True)
                            if sig2 != last_sub_sig and args2:
                                try:
                                    await ws.send(json.dumps({"op":"unsubscribe","args":args}))
                                except Exception:
                                ...
                                last_sub_sig = sig2
                                await ws.send(json.dumps({"op":"subscribe","args":args}))
                                logger.info("[ws-candles][okx] resubscribed topics=%s symbols=%s", len(args), len(syms2))
            except Exception as e:
                logger.warning("[ws-candles][okx] reconnect: %s", str(e)[:200])
                await asyncio.sleep(2.0)

    # run selected exchange loops
    tasks = []
    try:
        if "BINANCE" in exchanges:
            tasks.append(asyncio.create_task(_binance_loop(), name="ws-candles-binance"))
        if "BYBIT" in exchanges:
            tasks.append(asyncio.create_task(_bybit_loop(), name="ws-candles-bybit"))
        if "OKX" in exchanges:
            tasks.append(asyncio.create_task(_okx_loop(), name="ws-candles-okx"))

        if not tasks:
            logger.warning("[ws-candles] no exchanges enabled; exiting")
            return
        ...
        while True:
            ...
            if now - last_beat >= 60.0:
                ...
                try:
                    keys = len(buf)
                    pend = sum(1 for b in buf.values() if b.pending)
                    logger.info("[ws-candles] heartbeat keys=%s pending=%s symbols=%s tfs=%s market=%s ex=%s",
                                keys, pend, len(_get_symbols()), tfs, market, exchanges)
                except Exception:
                    pass
            # allow bot health hook
            try:
                cb = getattr(backend, "health_tick_cb", None)
                if callable(cb):
                    cb()
            except Exception:
                pass
